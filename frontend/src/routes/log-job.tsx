...
export const Route = createFileRoute("/log-job")({
  head: () => ({
    meta: [
      { title: "Log a Job — GigShield" },
      {
        name: "description",
        content: "Add a gig manually or drop a payout screenshot and let GigShield OCR read it.",
      },
      { property: "og:title", content: "Log a Job — GigShield" },
      {
    ...
    <div className="space-y-12">
      ...
      >
        ...
        <span className="relative inline-flex items-center gap-2 rounded-full border border-border bg-white/[0.04] px-3 py-1 text-[11px] text-muted-foreground">
          <Sparkles className="h-3 w-3 text-[#8B5CF6]" /> Powered by GigShield OCR
        </span>
...
function ChoiceCard({
  ...
  return (
    <motion.button
      ...
      className={cn(
        "group relative overflow-hidden rounded-3xl glass p-8 text-left transition-colors",
        ...
        {fields.map((f, i) => (
          ...
          >
            ...
            <Input
              ...
              className="h-12 rounded-2xl border-border bg-white/[0.04] px-4 text-sm focus-visible:ring-primary/40"
            />
          </motion.div>
        ))}
      </div>
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className={cn(
          "mt-8 cursor-pointer rounded-3xl border-2 border-dashed border-white/15 bg-white/[0.03] px-6 py-16 text-center transition-colors",
